...
def main():
    ...
    for scale in (1, 2, 3):
        name = f"compact-player-{scale}x.png"
        shutil.copyfile(args.captures / name, args.output / name)
    result = Image.open(args.output / "compact-player-2x.png").convert("RGB")
    reference = Image.open(args.reference).convert("RGB")
    assert reference.size == result.size
    overlay = Image.blend(reference, result, 0.5)
    overlay.save(args.output / "player-overlay-2x.png")
    ...
    for index, (title, image) in enumerate([
        ...
        ("Implemented Player — dotSpectrum / classic, production Metal", result),
        ("50% overlay", overlay),
    ]):
        y = 16 + index * (result.height + 36)
        draw.text((16, y), title, fill="#222222")
        sheet.paste(image, (16, y + 20))
    sheet.save(args.output / "player-comparison.png")
