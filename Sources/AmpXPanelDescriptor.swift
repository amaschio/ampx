...
struct AmpXPanelID: Hashable {
    ...
    static let equalizer = AmpXPanelID("equalizer")
    static let playlist = AmpXPanelID("playlist")
...
struct AmpXPanelDescriptor {
    let id: AmpXPanelID

    /// Whether the panel should currently be shown, read live from layout state.
    let isVisible: @MainActor () -> Bool

    /// Builds the panel's SwiftUI root. Classic AmpX chrome is applied inside this closure, so the
