...
struct ClassicModernSkin: AmpXSkin {
    ...
    func bevel(_ rect: CGRect, in context: CGContext, backingScale: CGFloat) {
        raisedFace(rect, style: .normal, in: context, backingScale: backingScale)
    }
    ...
    func displayWell(_ rect: CGRect, in context: CGContext, backingScale: CGFloat) {
        let well = snapped(rect, backingScale: backingScale)
        let unit = bandUnit(for: well)
        context.setFillColor(Palette.wellBlack.cgColor)
        context.fill(well)
        drawBands(in: well, unit: unit, context: context, edges: Edges(
    ...
    func panelFrame(_ rect: CGRect, contentFrame: CGRect?, in context: CGContext, backingScale: CGFloat) {
        let panelRect = snapped(rect, backingScale: backingScale)
        context.setFillColor(Palette.panel.cgColor)
        context.fill(panelRect)
        drawBands(in: panelRect, unit: 0.5, context: context, edges: Edges(
        ...
        let frame = snapped(contentFrame, backingScale: backingScale)
        ...
        drawBands(in: frame, unit: 0.5, context: context, edges: Edges(
    ...
    func raisedFace(_ rect: CGRect, style: AmpXFaceStyle, in context: CGContext, backingScale: CGFloat) {
        let face = snapped(rect, backingScale: backingScale)
        ...
        case .normal, .hovered:
            ...
            drawBands(in: face, unit: 0.5, context: context, edges: Edges(
    ...
    func headerRule(_ rect: CGRect, in context: CGContext, backingScale: CGFloat) {
        ...
        fillRows(in: snapped(upper, backingScale: backingScale), colors: [
            rgb(108, 116, 122), rgb(247, 250, 254), rgb(212, 193, 133), rgb(133, 93, 8),
            rgb(215, 194, 100), rgb(255, 255, 242), rgb(182, 185, 175),
        ], context: context)
        fillRows(in: snapped(lower, backingScale: backingScale), colors: [
    ...
    func sliderTrack(
        ...
        let track = snapped(rect, backingScale: backingScale)
        ...
        drawVerticalGradient(in: inner, stops: [
    ...
    func seekWell(_ well: CGRect, track: CGRect, in context: CGContext, backingScale: CGFloat) {
        let outer = snapped(well, backingScale: backingScale)
        let channel = snapped(track, backingScale: backingScale)
        context.setFillColor(rgb(17, 26, 44).cgColor)
        context.fill(outer)
        drawBands(in: outer, unit: 0.5, context: context, edges: Edges(
        ...
        fillRows(in: lip, colors: [rgb(73, 88, 116), rgb(79, 95, 125)] + Array(repeating: rgb(47, 60, 87), count: 4), context: context)

        context.setFillColor(rgb(12, 18, 33).cgColor)
        context.fill(channel)
        drawBands(in: channel, unit: 0.5, context: context, edges: Edges(
    ...
    func metallicThumb(_ rect: CGRect, material: AmpXThumbMaterial, in context: CGContext, backingScale: CGFloat) {
        let thumb = snapped(rect, backingScale: backingScale)
        switch material {
        case .steel:
            drawSteelThumb(thumb, context: context)
        case .gold:
            drawGoldThumb(thumb, context: context)
        }
    }
    ...
    private func drawSteelThumb(_ thumb: CGRect, context: CGContext) {
        ...
        context.clip()
        drawVerticalGradient(in: face, stops: [
