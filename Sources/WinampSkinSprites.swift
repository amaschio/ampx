...
enum WinampSkinSheet: String, CaseIterable {
    case main = "WinampSkinMain"
    case cbuttons = "WinampSkinCButtons"
    case shufrep = "WinampSkinShufrep"
    case posbar = "WinampSkinPosbar"
    case volume = "WinampSkinVolume"
    case balance = "WinampSkinBalance"
    case titlebar = "WinampSkinTitlebar"
    case numbers = "WinampSkinNumbers"
    case text = "WinampSkinText"
    case playpaus = "WinampSkinPlaypaus"
    case monoster = "WinampSkinMonoster"
    case eqmain = "WinampSkinEqmain"
    case pledit = "WinampSkinPledit"
}

/// Sprite rects in Webamp / Base 2.91 coordinates (top-left origin).
enum WinampSkinSprites {
    // MARK: - Main window
...
@MainActor
final class WinampSkinAtlas {
    static let shared = WinampSkinAtlas()

    private var sheets: [WinampSkinSheet: NSImage] = [:]
    ...
    private func loadSheet(_ sheet: WinampSkinSheet) -> NSImage? {
...
struct SkinSpriteView: View {
    ...
    var body: some View {
        if let image = WinampSkinAtlas.shared.swiftUIImage(for: self.sprite) {
            image
                .resizable()
                .interpolation(.none)
        ...
        Canvas { context, canvasSize in
            ...
            for char in sequence {
                guard let pos = ClassicMarqueeTypography.fontLookup[char],
                      let nsImage = WinampSkinAtlas.shared.image(
        ...
        HStack(spacing: 0) {
            ForEach(Array(self.text.enumerated()), id: \.offset) { _, char in
                if char == ":" {
                    ...
                    SkinSpriteView(sprite: WinampSkinSprites.Numbers.minus, scale: self.scale)
                        .frame(width: 5 * self.scale, height: 2 * self.scale)
                } else if let digit = WinampSkinSprites.Numbers.digit(char) {
...
struct ClassicBitmapSeekBar: View {
    ...
    var body: some View {
        GeometryReader { geo in
            let thumbW = WinampSkinSprites.Posbar.thumb.width * self.scale
            ...
            ZStack(alignment: .leading) {
                SkinSpriteView(sprite: WinampSkinSprites.Posbar.background, scale: self.scale)
                    .frame(width: geo.size.width, height: geo.size.height)
                SkinSpriteView(sprite: WinampSkinSprites.Posbar.thumb, scale: self.scale)
                    .offset(x: x)
            }
            .contentShape(Rectangle())
            .gesture(
...
struct ClassicBitmapPlayState: View {
    ...
    private var sprite: Sprite {
        if self.audioPlayer.isPlaying {
            WinampSkinSprites.PlayPaus.playing
        } else if self.audioPlayer.duration > 0 {
            WinampSkinSprites.PlayPaus.paused
        } else {
            WinampSkinSprites.PlayPaus.stopped
        }
    }
        ...
        HStack(spacing: 0) {
            SkinSpriteView(
                sprite: self.audioPlayer.currentChannels == 1
                    ? WinampSkinSprites.MonoSter.monoOn
                    : WinampSkinSprites.MonoSter.mono,
                scale: self.scale
            )
            SkinSpriteView(
                sprite: self.audioPlayer.currentChannels >= 2
                    ? WinampSkinSprites.MonoSter.stereoOn
                    : WinampSkinSprites.MonoSter.stereo,
                scale: self.scale
            )
        }
    }
...
struct ClassicBitmapWindowButton: View {
    ...
    private var sprites: (normal: Sprite, pressed: Sprite) {
        switch self.kind {
        case .options: (WinampSkinSprites.Titlebar.options, WinampSkinSprites.Titlebar.optionsPressed)
        case .minimize: (WinampSkinSprites.Titlebar.minimize, WinampSkinSprites.Titlebar.minimizePressed)
        case .shade: (WinampSkinSprites.Titlebar.shade, WinampSkinSprites.Titlebar.shadePressed)
        case .unshade: (WinampSkinSprites.Titlebar.unshade, WinampSkinSprites.Titlebar.unshadePressed)
        case .close: (WinampSkinSprites.Titlebar.close, WinampSkinSprites.Titlebar.closePressed)
        }
    }
