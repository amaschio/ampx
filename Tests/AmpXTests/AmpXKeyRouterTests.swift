...
final class AmpXKeyRouterTests: XCTestCase {
    ...
    func testPlaylistArrowKeysRouteToPlaylistWhenModuleFocused() {
        ...
        XCTAssertEqual(
            AmpXKeyRouter.route(event: keyDown(keyCode: 126), context: context),
            .playlist
        )
        XCTAssertEqual(
    ...
    func testPlaylistBindingsPreserveModifiers() {
        ...
        XCTAssertEqual(
            AmpXKeyRouter.route(
                event: keyDown(keyCode: 0, modifierFlags: [.command]),
                context: context
            ),
            .playlist
        )
        XCTAssertEqual(
            AmpXKeyRouter.route(
                event: keyDown(keyCode: 126, modifierFlags: [.option]),
                context: context
            ),
            .playlist
        )
        XCTAssertEqual(
            AmpXKeyRouter.route(
                event: keyDown(keyCode: 126, modifierFlags: [.control]),
                context: context
            ),
            .unhandled
        )
    ...
    func testEntheaKeysRouteToEntheaModule() {
        ...
        XCTAssertEqual(
            AmpXKeyRouter.route(event: keyDown(keyCode: 3), context: context),
            .enthea
        )
        XCTAssertEqual(
            AmpXKeyRouter.route(event: keyDown(keyCode: 53), context: context),
            .enthea
        )
    }
    ...
    func testTextResponderBypassesGlobalSingleLetterShortcuts() {
        ...
        XCTAssertEqual(
            AmpXKeyRouter.route(event: keyDown(keyCode: 123), context: context),
            .global
        )
    }
    ...
    func testModuleCommandKeysAreRecognized() {
        XCTAssertEqual(
            AmpXKeyRouter.moduleCommand(for: keyDown(keyCode: 126, modifierFlags: [.command, .option])),
            .moveUp
        )
        XCTAssertEqual(
            AmpXKeyRouter.moduleCommand(for: keyDown(keyCode: 125, modifierFlags: [.command, .option])),
            .moveDown
        )
        XCTAssertEqual(
            AmpXKeyRouter.moduleCommand(for: keyDown(keyCode: 2, modifierFlags: [.command, .option])),
            .toggleDetach
        )
        XCTAssertEqual(
            AmpXKeyRouter.moduleCommand(for: keyDown(keyCode: 8, modifierFlags: [.command, .option])),
            .toggleCollapse
        )
    }
    ...
    func testModuleCommandsDoNotRouteToGlobal() {
        let context = AmpXFocusContext(module: .player, control: nil)
        let event = keyDown(keyCode: 8, modifierFlags: [.command, .option])
