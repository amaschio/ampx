...
final class AmpXCompactPlayerTests: XCTestCase {
    func testFiveTransportButtonsCallOnlyTheirCorrespondingModelAction() {
        for index in 0 ..< 5 {
            ...
            let playlist = CompactPlaylistSpy(audioPlayer: MockAudioPlayer(), restoreBookmarks: false,
            ...
            XCTAssertEqual([playlist.previousCount, audio.playCount, audio.pauseCount, audio.stopCount, playlist.nextCount],
                           (0 ..< 5).map { $0 == index ? 1 : 0 })
        }
    }

    func testMeasuredControlsStayInsideTheirCellsAndCannotOverlapChrome() {
        let audio = CompactAudioSpy(installRemoteCommands: false)
        let playlist = CompactPlaylistSpy(audioPlayer: MockAudioPlayer(), restoreBookmarks: false,
                                           restorePlaylist: false, alertPresenter: SilentPlaylistAlertPresenter())
        let player = self.makePlayer(audio: audio, playlist: playlist)
    ...
    func testCompactVisualizerAndTimerShareStateWithExpandedViews() {
        let audio = CompactAudioSpy(installRemoteCommands: false)
        let playlist = CompactPlaylistSpy(audioPlayer: MockAudioPlayer(), restoreBookmarks: false,
                                           restorePlaylist: false, alertPresenter: SilentPlaylistAlertPresenter())
        let player = self.makePlayer(audio: audio, playlist: playlist)
        ...
        XCTAssertTrue(self.sharedState!.showRemainingTime)
    }
...
private final class CompactAudioSpy: AudioPlayer {
    ...
    var pauseCount = 0
    var stopCount = 0
    override func playOrResume() { self.playCount += 1 }
    override func pause() { self.pauseCount += 1 }
    override func stop() { self.stopCount += 1 }
}
...
private final class CompactPlaylistSpy: PlaylistManager {
    var previousCount = 0
    var nextCount = 0
    override func previous() { self.previousCount += 1 }
    override func next() { self.nextCount += 1 }
}
